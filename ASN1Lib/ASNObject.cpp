...
{
    if (tag>31 || tag<0) throw argument_error("Invalid tag number");
    if (length<0 || length>127) throw argument_error("Invalid length");
    vector<char> data;
...
void ASNObject::taglength(const vector<char> &code)
{
    ...
    if(isIndefinite)
        if(any_of(code.begin()+9, code.end(), [](char i){ return i!='0'; } ))
            throw argument_error("Indefinite length set but bits encoding length are not 0s");

    bitset<5> bintag;
    for(int i=3, j=4; i<8; i++, j--)
        bintag.set(j, code[i]-'0');
    if((unsigned)tag != bintag.to_ulong()) throw argument_error("Tag doesn't match the object's type");
